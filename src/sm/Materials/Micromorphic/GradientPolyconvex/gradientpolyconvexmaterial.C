...
#include "../sm/Materials/Micromorphic/Micropolar/gradientpolyconvexmaterial.h"
...
GradientPolyconvexMaterial :: giveMicromorphicMatrix_dSigdPhi(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
   ...
   for (int i = 1; i <= 3; i++) {
	...
	for (int o = 1; m <=3; m++) {
	  for (int p = 1; n<=3; n++) {
	    answer.at(giveVI(i,j),giveVI(o,p)) = (vInvF.at(giveVI(j,i))*vInvF.at(giveVI(n,m))-vInvF.at(giveVI(n,i))*vInvF.at(giveVI(j,m))) * J * sCof.at(giveVI(m,n));
	  }
	}
      }
    }
...
GradientPolyconvexMaterial :: giveFiniteStrainGeneralizedStressVectors(FloatArray &sigma, FloatArray &s, FloatArray &M, GaussPoint *gp, const FloatArray &vF, const FloatArray &micromorphicVar, const FloatArray micromorphicVarGrad, TimeStep *tStep)
{
    ...
    mvCofF.beSubArrayOf(micromorphicVar, {2,3,4,5,6,7,8,9,10});
    sCof = vCofF;
    sCof.subtract(mvCof);
    sCof.times(-Hk);
    // relative stress for determinatn
    double sDet = -Hk * (det - micromorphicVar.at(1));
